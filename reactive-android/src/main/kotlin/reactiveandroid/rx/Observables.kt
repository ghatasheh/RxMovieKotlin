...
public fun <T : Tuple2<A, B>, A, B, R> Observable<T>.reduce(next: (A, B) -> R) : Observable<R> =
        map { next(it.component1(), it.component2()) }

public fun <T : Tuple3<A, B, C>, A, B, C, R> Observable<T>.reduce(next: (A, B, C) -> R) : Observable<R> =
        map { next(it.component1(), it.component2(), it.component3()) }

public fun <T : Tuple4<A, B, C, D>, A, B, C, D, R> Observable<T>.reduce(next: (A, B, C, D) -> R) : Observable<R> =
        map { next(it.component1(), it.component2(), it.component3(), it.component4()) }
...
public fun <T : Tuple3<A, B, C>, A, B, C> Observable<T>.subscribe(next: (A, B, C) -> Unit, error: ((e: Throwable?) -> Unit)? = null, complete: (() -> Unit)? = null): Subscription {
    return subscribe(object : Subscriber<T>() {
...
public fun <T : Tuple4<A, B, C, D>, A, B, C, D> Observable<T>.subscribe(next: (A, B, C, D) -> Unit, error: ((e: Throwable?) -> Unit)? = null, complete: (() -> Unit)? = null): Subscription {
    return subscribe(object : Subscriber<T>() {
